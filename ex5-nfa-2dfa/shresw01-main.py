...
class DFA:
    ...
    def __init__(self, classes=orderedcollections.OrderedMap(), states=orderedcollections.OrderedMap()):
        ...
        self.numStates = len(states) # ordered map
    ...
    def buildFromNFA(self, nfa):
        ...
        def getAcceptingTokenId(stateSet):
            ...
            for stateid in stateSet:
                condition = nfa.states[stateid].isAccepting()   #  If the state is accepting, then it returns to 'accepting' token "Yes"
                if condition:
        ...
        def EPSclosure(stateSet):
            ...
            while len(unexploredStates) != 0:
                ...
                for toStateID in toStates:  # This takes a depth first search approach
        ...
        def gatherClasses(states):
            ...
            classesGathered = orderedcollections.OrderedSet()

            for stateID in states:
                transitions = nfa.states[stateID].getTransitions()
                for onClass in transitions:
                    if onClass != epsilon:
                        classesGathered.add(onClass)

            return classesGathered
